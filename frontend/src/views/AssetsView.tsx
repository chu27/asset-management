...
import type { AssetRow, AssetSummary, Currency } from "../types";

const currencies: Currency[] = ["JPY", "USD", "CNY"];
const purple = ["#6f4bb3", "#9d7ad6", "#d7ccf0"];
const currencyColors: Record<Currency, string> = { JPY: "#ff7417", USD: "#1eb85f", CNY: "#f84444" };
const typeLabel: Record<string, string> = { stock: "股票", cash: "现金", deposit: "银行存款" };
...
export default function AssetsView() {
  ...
  async function saveAsset(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api(editing ? `/api/assets/${editing.id}` : "/api/assets", { method: editing ? "PUT" : "POST", body: JSON.stringify({ name: form.get("name"), asset_type: form.get("type"), institution: form.get("institution"), currency: form.get("currency"), balance: Number(form.get("balance")), linked: form.get("linked") === "yes", note: form.get("note") }) }); setShowAdd(false); setEditing(null); setMessage(editing ? "资产已更新" : "资产已添加"); await load(); } catch (error) { setMessage((error as Error).message); }
  }
  ...
  return <>
    <section className="page-intro portfolio-actions-only"><div className="intro-actions"><button className="btn secondary" onClick={refreshRates}>刷新汇率</button><button className="btn primary" onClick={() => setShowAdd(true)}>＋ 添加资产</button></div></section>
    ...
    <div className="asset-stack">{currencies.map((currency) => <section className="card currency-detail" ref={(element) => { detailRefs.current[currency] = element; }} key={currency}><div className="card-heading"><div><h2>{currency}资产明细</h2><p>{data.rows.filter((item) => item.currency === currency).length} 项资产</p></div><strong>{format(data.currency_totals[currency], currency)}</strong></div><div className="table-wrap"><table><thead><tr><th>资产名称</th><th>类型</th><th>所属机构</th><th>当前金额</th><th>来源</th><th /></tr></thead><tbody>{data.rows.filter((item) => item.currency === currency).map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small className="block">{item.secondary}</small></td><td>{typeLabel[item.asset_type]}</td><td>{item.institution}</td><td>{format(item.balance, currency)}</td><td><span className={`source ${item.source}`}>{item.source === "auto" ? "自动同步" : "手动填写"}</span></td><td>{item.source === "manual" && <div className="row-actions"><button onClick={() => setEditing(item)}>修改</button><button className="text-danger" onClick={async () => { if (!confirm("确认删除这项资产？")) return; await api(`/api/assets/${item.id}`, { method: "DELETE" }); await load(); }}>删除</button></div>}</td></tr>)}</tbody></table></div></section>)}</div>
    {(showAdd || editing) && <Modal title={editing ? "修改资产" : "添加资产"} subtitle="现金或银行存款" onClose={() => { setShowAdd(false); setEditing(null); }}><form className="form-grid" onSubmit={saveAsset}><label className="full">资产名称<input name="name" defaultValue={editing?.name ?? ""} required /></label><label>资产类型<select name="type" defaultValue={editing?.asset_type ?? "cash"}><option value="cash">现金</option><option value="deposit">银行存款</option></select></label><label>币种<select name="currency" defaultValue={editing?.currency ?? "JPY"}><option>JPY</option><option>USD</option><option>CNY</option></select></label><label>所属机构<input name="institution" defaultValue={editing?.institution === "—" ? "" : editing?.institution ?? ""} /></label><label>当前余额<input name="balance" type="number" step="any" min="0" defaultValue={editing?.balance ?? 0} /></label><label>关联投资记录<select name="linked" defaultValue={editing?.linked ? "yes" : "no"}><option value="no">不关联</option><option value="yes">关联</option></select></label><label className="full">备注<textarea name="note" defaultValue={editing?.secondary ?? ""} /></label><div className="form-actions full"><button type="button" className="btn secondary" onClick={() => { setShowAdd(false); setEditing(null); }}>取消</button><button className="btn primary">{editing ? "保存修改" : "保存资产"}</button></div></form></Modal>}
  </>;
}
